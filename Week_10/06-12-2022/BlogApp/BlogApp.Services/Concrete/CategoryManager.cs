...
namespace BlogApp.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        public async Task<IDataResult<CategoryUpdateDto>> GetCategoryUpdateDto(int categoryId)
        {
            ...
            if (result)
            {
                var category = await _unitOfWork.Categories.GetAsync(c => c.Id == categoryId);//kategori tipinde deger var
                var categoryUpdateDto = _mapper.Map<CategoryUpdateDto>(category);   // category tipindeki degeri al categoryUpdaateDto tipindeki degere dönüştür ve bir değişkene at.
                return new DataResult<CategoryUpdateDto>(ResultStatus.Success, categoryUpdateDto);//ResultStatus ve dto yu gönderdik
            }
            else
            {
