$(document).ready(function () {
            ...
            },
            {
                ...
                action: function (e, dt, node, config) {
                    ...
                    $.ajax({
                        ...
                        //dışardan gelen datayla çalışıoruz
                        success: function (data) {
                            const categoryListDto = jQuery.parseJSON(data);
                            //0 success denk geliyor başarılıysa tabloyu temizle yeniden verileri göster.
                            //categoryListDto.Categories.$values dönüyor
                            if (categoryListDto.ResultStatus === 0) {
                                let tableBody = '';
                                $.each(categoryListDto.Categories.$values,
                                    function (index, category) {
                                        console.log("Kategoriler: ", category);
                                        //Her defasında yeni bir tr oluştur ekle
                                        tableBody += `
                <tr name="${category.Id}">
                    <td>${category.Id}</td>
                    <td>${category.Name}</td>
                    ...
                    <td>
                        ...
                        <button class="btn btn-danger btn-sm btn-delete" data-id="${category.Id}"><span class="fas fa-minus-circle"></span></button>
                                ...
                                //categoriestabledaki  t bodyi table bodyle değiştir
    ...
    $(function () {
        ...
        $('#btnAdd').click(function () {
            //get metoduyla url bilgisi  category controller partial view  yükle ve çalıştır.
            $.get(url).done(function (data) {
                //placeHolderDiv içine yerleştir
                placeHolderDiv.html(data);
                placeHolderDiv.find(".modal").modal('show');
                //Ekranda kaydetme formunu gösterme
            });
        });
        //Ajax'la (GET) partial viewimizi ekrana getirme BİTTİ

        //Ajax'la POST işlemleri
        //Id si btn save olana click olayla yakala event al default kullanmak için
        placeHolderDiv.on('click', '#btnSave', function (event) {

            event.preventDefault();
            //form için değişken tanımla category add partical da görebiliriz 4 tane bilgi veriyoruz orda ve bir input var value controllerlardan viewe veri almak için ViewBag
            //ModalStade formda girilen bilgilerin içinde tutulduğu bir yapı kuralları kontrol eder boş bırakılmamalı gibi doğruysa true yu değilse false döndrülüyor

            const form = $('#form-category-add');
            const actionUrl = form.attr('action');
            const dataToSend = form.serialize();
            //post bizden url istiyor dataToSend categoryDto alıyor 
            $.post(actionUrl, dataToSend).done(function (data) {
                //data bilgisini çevirdik
                const categoryAddAjaxModel = jQuery.parseJSON(data);
                //newFormBody tekrar gelen datalarla göster 
    ...
    $(function () {
        ...
        placeHolderDiv.on('click', '#btnupdate', function (event) {
            ...
            const actionUrl = form.attr('action');
            const dataToSemd = form.serialize();
                    ...
                    if (isValid) {
                        ...
                        if (!categoryUpdateAjaxModel.CategoryDto.Category.IsDeleted) {
                          ...
                          <tr name="${categoryAddAjaxModel.CategoryDto.Category.Id}">
                            ...
                            toastr.success(`${categoryUpdateAjaxModel.CategoryDto.Category.Name}adlı kategori silindi.`, 'işlem Başarılı!')
                        }
                    }
                    else {
                        let summaryText = '<br>'
                        $('#validation-summary>ul>li').each(function () {
                            let text = $(this).text();
                            toastr.error(text);
                            //üç tane ayrı ayrı toastr yapmamızı sağladı.
                        });
                    }
                })
        })
