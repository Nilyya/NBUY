...
namespace DiariesFood.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginDto loginDto)
        {
            if (ModelState.IsValid)
            {
               ...
               if (!await _userManager.IsEmailConfirmedAsync(user))
                {
                    TempData["Message"] = Jobs.PushMessage("Bilgi", "Hesabınız onaylanmamış mailinize gelen onay linkine tıklayınız", "warning");
                    return View(loginDto);
                }
        ...
        public async Task<IActionResult> Register(RegisterDto registerDto)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    TempData["Message"] = Jobs.PushMessage("Bilgi", "Hesabınız başarıyla oluşturuldu", "success");
        ...
        public async Task<IActionResult> ConfirmEmail(string userId, string token)
        {
            ...
            {
                TempData["Message"] = Jobs.PushMessage("Hata", "Geçersiz token ya da user bilgisi.", "danger");
                return View();
            }
            ...
            if (user != null)
            {
                ...
                if (result.Succeeded)
                {
                    TempData["Message"] = Jobs.PushMessage("Başarılı", "Hesabınız başarıyla onaylandı. Giriş yapabilirsiniz.", "success");
                    return RedirectToAction("Login", "Account");

                }
            }
            TempData["Message"] = Jobs.PushMessage("Bilgi", "Bir sorun oluştu ve hesabınız onaylanmadı. Tekrar deneyiniz.", "danger");
            return View();
        }
        ...
        public async Task<IActionResult> ForgotPas(string email)
        {
            ...
            {
                TempData["Message"] = Jobs.PushMessage("Hata", "Lütfen mail adresinizi eksiksiz ve doğru girin", "danger");
                return View();
            }
            var user = await _userManager.FindByEmailAsync(email);
            if (user == null)
            {
                TempData["Message"] = Jobs.PushMessage("Hata", "Böyle bir mail adresi bulunamadı lüften tekrar deneyin", "danger");
                return View();
            }
            ...
            TempData["Message"] = Jobs.PushMessage("Bilgi", "Şifre sıfırlama linkiniz mailinize gönderildi lütfen mail adresinizi kontrol ediniz", "info");
            return RedirectToAction("Login", "Account");
        }
        ...
        public IActionResult ResetPas(string userId, string token)
        {
            if (userId == null || token == null)
            {
                TempData["Message"] = Jobs.PushMessage("Hata", "Bir sorun oluştu lütfen daha sonra tekrar deneyiniz", "danger");
                return RedirectToAction("Index", "Home");
            }
        ...
        public async Task<IActionResult> ResetPas(ResetPasDto resetPasDto)
        {
            ...
            if (user == null)
            {
                TempData["Message"] = Jobs.PushMessage("Hata", "Böyle bir kullanıcı bulunamadı", "danger");
                return View(resetPasDto);
            }
            var result = await _userManager.ResetPasswordAsync(user, resetPasDto.Token, resetPasDto.Password);
            if (result.Succeeded)
            {
                TempData["Message"] = Jobs.PushMessage("Başarılı", "Parolanız başarıyla değiştirildi giriş yapabilrsiniz", "success");
                return RedirectToAction("Login", "Account");
            }
            TempData["Message"] = Jobs.PushMessage("Hata", "Bir hata oluştu", "danger");
            return Redirect("~/");
        }
        ...
        public async Task<IActionResult> Manage(UserManageDto userManageDto)
        {
            ...
            if (result.Succeeded)
            {
                TempData["Message"] = Jobs.PushMessage("Başarılı!", "Profiliniz başarıyla kaydedilmiştir.", "success");
            }
            List<SelectListItem> genderList = new List<SelectListItem>();
            genderList.Add(new SelectListItem
            {
